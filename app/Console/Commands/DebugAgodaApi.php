...
class DebugAgodaApi extends Command
{
    protected $signature = 'debug:agoda-api {location?}';
    protected $description = 'Debug the Agoda API integration with detailed output';

    public function handle()
    {
        $location = $this->argument('location') ?? 'Paris';
        $this->info("Debugging Agoda API for location: {$location}");
        ...
        $this->info("\nTesting API calls with debugging:");
        $service = new HotelRecommendationService();
        
        try {
            // Let's manually test the city suggestions first
            $this->info("\n1. Testing city suggestions:");
            $suggestionResponse = \Illuminate\Support\Facades\Http::timeout(10)
                ->withHeaders([
            ...
            $this->info("City suggestions response status: " . $suggestionResponse->status());
            $this->info("City suggestions response body: " . $suggestionResponse->body());

            if (!$suggestionResponse->successful()) {
                $this->error("Failed to get city suggestions");
                return;
            }

            $suggestions = $suggestionResponse->json();
            if (empty($suggestions) || !is_array($suggestions)) {
                $this->error("No city suggestions found");
                return;
            }

            $this->info("Found " . count($suggestions) . " city suggestions");
            foreach ($suggestions as $index => $suggestion) {
                $this->info("  $index. " . ($suggestion['DisplayText'] ?? 'Unknown') . " (ID: " . ($suggestion['Value'] ?? 'Unknown') . ")");
            }

            $cityId = $suggestions[0]['Value'] ?? null;
            if (!$cityId) {
                $this->error("City ID not found in suggestions");
                return;
            }

            $this->info("Using city ID: $cityId");

            // Now test the hotel search
            ...
            $hotelResponse = \Illuminate\Support\Facades\Http::timeout(30)
                ->withHeaders([
            ...
            $this->info("Hotel search response body: " . $hotelResponse->body());

            if (!$hotelResponse->successful()) {
                $this->error("Failed to get hotels from Agoda");
                return;
            }
            ...
            $hotels = $service->getHotelRecommendations($location, null, null, 'medium');
            
            $this->info("Service method returned " . count($hotels) . " hotels");
            ...
            if (!empty($hotels)) {
                ...
                $this->error("No hotels returned from service method");
            }
        } catch (\Exception $e) {
            $this->error("Error testing API: " . $e->getMessage());
            $this->error("Trace: " . $e->getTraceAsString());
            Log::error('Agoda API debug failed', [
                'error' => $e->getMessage(),
